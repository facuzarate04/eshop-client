<template>
    <header class="flex justify-between items-center p-6 border-b-2">
        ...
        <div class="flex items-center space-x-4">
            <button @click="changeDarkMode"
                ...
                <svg v-show="darkMode" class="h-6 w-6" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div class="relative z-50">
                ...
                <div class="absolute right-0 mt-2 w-48 bg-white rounded-md overflow-hidden shadow-xl z-10"
                    v-show="dropdownOpen" x-transition:enter="transition ease-out duration-100 transform"
                    ...
                    <button @click.prevent="logout"
                        class="block px-4 py-2 text-sm text-gray-700 hover:bg-purple-600 hover:text-white">Logout</button>
                </div>
            </div>
        </div>
    </header>
...
export default {
    ...
    methods: {
        ...
        logout() {
            axios.post(ApiRoutes.logout.url).then((response) => {
